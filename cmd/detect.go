...
func runDetect(cmd *cobra.Command, args []string) {
	...
	exceptionsPath, _ := cmd.Flags().GetString("exceptions") 
	if exceptionsPath != "" {
		err = detector.AddExceptions(exceptionsPath)
		if err != nil {
			log.Error().Msgf("Could not load exceptions: %s", err)
			// Continue without exceptions rather than failing
		} else {
			log.Info().Msgf("Loaded exceptions from: %s", exceptionsPath)
		}
	}
	...
	gateSpecs, _ := cmd.Flags().GetStringSlice("quality-gate") 
	if len(gateSpecs) > 0 {
		gates, err := ParseQualityGates(gateSpecs)
		if err != nil {
			log.Fatal().Err(err).Msg("Invalid quality gates")
		}

		gateResult := CheckQualityGates(findings, gates)
		PrintQualityGateSummary(gateResult)

		if !gateResult.Passed {
			// Quality gates failed - exit with error code
			log.Error().Msg("Quality gates failed - stopping pipeline")
			os.Exit(exitCode)
		}
	}
