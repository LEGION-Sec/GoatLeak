...
type Detector struct {
	...
	//exceptions for marking findings as exceptions
...
func NewDetectorDefaultConfig() (*Detector, error) {
	return nil, fmt.Errorf("default config is not supported. Use --config flag to specify a configuration file")
}
...
// AddExceptions loads exceptions from a JSON file. REVVITY
func (d *Detector) AddExceptions(exceptionsPath string) error {
	if exceptionsPath != "" {
		exceptions, err := report.LoadExceptions(exceptionsPath)
		if err != nil {
			return err
		}
		d.exceptions = exceptions
		log.Debug().Msgf("Loaded %d exceptions from %s", len(exceptions), exceptionsPath)
	}
	return nil
}
...
func (d *Detector) detectRule(fragment Fragment, rule config.Rule) []report.Finding {
	...
	log.Debug().Msgf("detectRule - Rule: %s, Severity: %s", rule.RuleID, rule.Severity)
	...
	if rule.Path != nil && rule.Regex == nil {
		...
		if rule.Path.Match([]byte(fragment.FilePath)) {
			finding := report.Finding{
				...
				Tags:        rule.Tags,
				Severity:    rule.Severity, // REVVITY
			}
...
func (d *Detector) Detect(fragment Fragment) []report.Finding {
	...
	if len(d.exceptions) > 0 {
		findings = report.MarkExceptions(findings, d.exceptions) 
	}
...
func (d *Detector) addFinding(finding report.Finding) {
	if len(d.exceptions) > 0 && report.IsException(finding, d.exceptions) { 
		finding.Severity = "Exception"
	}
