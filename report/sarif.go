...
func mapSeverityToSARIF(severity string) string {
    ...
    case "low":
        return "note"
    default:
        return "warning" // default to warning if severity is not set or invalid
    }
}
...
func getRules(cfg config.Config) []Rules {
	...
	for _, rule := range cfg.OrderedRules() {
		...
		rules = append(rules, Rules{
			...
			DefaultConfiguration: DefaultConfiguration{ // REvvity
                Level: mapSeverityToSARIF(rule.Severity), // REvvity
            },
		})
	}
	return rules
...
type Results struct {
	...
	Level               string      `json:"level,omitempty"` // REvvity
